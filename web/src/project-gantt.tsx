 ...
 useEffect(()=>{
  ...
  const onPointerUp=(e:PointerEvent)=>{
   setDrag(current=>{
    ...
    if(!hasMoved){open(current.task);}
    else if(deltaDays!==0){
     const result=calculateDragDates(current.initialStart,current.initialDate,current.mode,deltaDays);
     if(result.changed){onUpdate(current.task.id,{start:result.start,date:result.date});}
    }
    return null;
   });
  };
